        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-8">
                ...
                <p class="text-gray-600 dark:text-gray-400 mt-1">
                    Informations complètes sur la classe.
                </p>
            </div>
            ...
            <div class="glass rounded-2xl border border-gray-200 dark:border-gray-700/50 p-6 sm:p-8">
                <div class="flex flex-col md:flex-row md:items-start gap-8">
                    <div class="flex-shrink-0">
                        <div
                            class="w-20 h-20 rounded-2xl bg-gradient-to-br from-indigo-500 to-blue-600 flex items-center justify-center text-white font-bold text-3xl shadow-lg shadow-indigo-500/30">
                            {{ substr($classe->CodeC, 0, 2) }}
                        </div>
                    </div>

                    <div class="flex-grow grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            <div class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Code Classe</div>
                            <div class="text-lg font-semibold text-gray-900 dark:text-white">{{ $classe->CodeC }}</div>
                        </div>

                        <div>
                            <div class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Libellé</div>
                            <div class="text-lg font-semibold text-gray-900 dark:text-white">{{ $classe->Libelle }}
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-2">
                                <span
                                    class="text-lg font-semibold text-gray-900 dark:text-white">{{ $classe->CodeD }}</span>
                                @if ($classe->departement?->Libelle)
                                    <span
                                        class="text-sm text-gray-500 dark:text-gray-400">({{ $classe->departement->Libelle }})</span>
                                @endif
                            </div>
                        </div>

                        <div>
                            <div class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Séances programmées
                            </div>
                            <div class="text-lg font-semibold text-gray-900 dark:text-white">
                                {{ $classe->cours?->count() ?? 0 }}</div>
                        </div>
                    </div>
                </div>
