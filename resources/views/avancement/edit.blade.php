        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-8">
                ...
                <h2 class="text-3xl font-display font-bold text-gray-900 dark:text-white">
                    {{ __('Modifier l\'avancement') }}
                </h2>
                <p class="text-gray-600 dark:text-gray-400 mt-1">
                    Mettez à jour les informations de l'avancement.
                </p>
            </div>

            <div class="glass rounded-2xl border border-gray-200 dark:border-gray-700/50 p-6 sm:p-8">
                <form method="POST"
                    action="{{ route('avancement.update', [$avancement->CodeE, $avancement->CodeC, $avancement->CodeM]) }}"
                    class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="md:col-span-2">
                            ...
                            <select id="CodeE" name="CodeE"
                                ...
                                @foreach ($enseignants as $enseignant)
                                    <option value="{{ $enseignant->CodeE }}" @selected(old('CodeE', $avancement->CodeE) == $enseignant->CodeE)>
                        ...
                        <div>
                            ...
                            <select id="CodeC" name="CodeC"
                                ...
                                @foreach ($classes as $classe)
                                    <option value="{{ $classe->CodeC }}" @selected(old('CodeC', $avancement->CodeC) == $classe->CodeC)>
                        ...
                        <div>
                            ...
                            <x-text-input id="MHRealise" name="MHRealise" type="number" step="0.01" min="0"
                                class="mt-1 block w-full" :value="old('MHRealise', $avancement->MHRealise)" required />
                        ...
                        <div class="md:col-span-2">
                            ...
                            <select id="CodeM" name="CodeM"
                                ...
                                @foreach ($matieres as $matiere)
                                    <option value="{{ $matiere->CodeM }}" @selected(old('CodeM', $avancement->CodeM) == $matiere->CodeM)>
