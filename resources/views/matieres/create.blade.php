        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-8">
                ...
                <p class="text-gray-600 dark:text-gray-400 mt-1">
                    Ajoutez une nouvelle matière au programme académique.
                </p>
            </div>
            ...
            <div class="glass rounded-2xl border border-gray-200 dark:border-gray-700/50 p-6 sm:p-8">
                <form method="POST" action="{{ route('matieres.store') }}" class="space-y-6">
                    ...
                    <div>
                        <x-input-label for="CodeM" :value="__('Code matière')"
                            class="text-gray-700 dark:text-gray-300 font-medium mb-1" />
                        <x-text-input id="CodeM" name="CodeM" type="text" class="mt-1 block w-full"
                            :value="old('CodeM')" required autofocus placeholder="Ex: MATH101" />
                    ...
                    <div>
                        ...
                        <x-text-input id="Libelle" name="Libelle" type="text" class="mt-1 block w-full"
                            :value="old('Libelle')" required placeholder="Ex: Algèbre Linéaire" />
                        <x-input-error class="mt-2" :messages="$errors->get('Libelle')" />
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <x-input-label for="MH" :value="__('Masse horaire (MH)')"
                                class="text-gray-700 dark:text-gray-300 font-medium mb-1" />
                            <x-text-input id="MH" name="MH" type="number" step="0.01" min="0" class="mt-1 block w-full"
                                :value="old('MH')" required placeholder="Ex: 45" />
                            <x-input-error class="mt-2" :messages="$errors->get('MH')" />
                        </div>
                        <div>
                            <x-input-label for="Coef" :value="__('Coefficient')"
                                class="text-gray-700 dark:text-gray-300 font-medium mb-1" />
                            <select id="Coef" name="Coef"
                                ...
                                required>
                                <option value="" disabled @selected(old('Coef') === null)>—</option>
                                @foreach ([1, 3, 5] as $coef)
                                    <option value="{{ $coef }}" @selected(old('Coef') == $coef)>{{ $coef }}</option>
                                @endforeach
                            </select>
                            <x-input-error class="mt-2" :messages="$errors->get('Coef')" />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <select id="CodeD" name="CodeD"
                            class="mt-1 block w-full border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900/50 dark:text-gray-300 focus:border-primary-500 dark:focus:border-primary-600 focus:ring-primary-500 dark:focus:ring-primary-600 rounded-xl shadow-sm"
                            required>
                            <option value="" disabled @selected(old('CodeD') === null)>Sélectionnez un département
                            </option>
                            @foreach ($departements as $departement)
                                <option value="{{ $departement->CodeD }}" @selected(old('CodeD') == $departement->CodeD)>
                    ...
                    <div class="flex items-center justify-end gap-4 pt-4 border-t border-gray-100 dark:border-gray-700">
                        ...
                        <x-primary-button>{{ __('Créer la matière') }}</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
