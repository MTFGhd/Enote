            ...
            <div class="glass rounded-2xl border border-gray-200 dark:border-gray-700/50 p-6 sm:p-8">
                <form method="POST" action="{{ route('matieres.update', $matiere) }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <x-text-input id="CodeM" type="text"
                            class="mt-1 block w-full bg-gray-50 dark:bg-gray-800 text-gray-500 cursor-not-allowed"
                            :value="$matiere->CodeM" disabled />
                        <p class="mt-1 text-xs text-gray-500">Le code matière ne peut pas être modifié.</p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <x-text-input id="MH" name="MH" type="number" step="0.01" min="0" class="mt-1 block w-full"
                                :value="old('MH', $matiere->MH)" required />
                        ...
                        <div>
                            ...
                            <select id="Coef" name="Coef"
                                ...
                                @foreach ([1, 3, 5] as $coef)
                                    <option value="{{ $coef }}" @selected(old('Coef', $matiere->Coef) == $coef)>{{ $coef }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <select id="CodeD" name="CodeD"
                            ...
                            @foreach ($departements as $departement)
                                <option value="{{ $departement->CodeD }}" @selected(old('CodeD', $matiere->CodeD) == $departement->CodeD)>
